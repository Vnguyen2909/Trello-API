...
const createNewBoardInvitation = async (req, res, next) => {
  try {
    //User thuc hien request nay chinh la Inviter - nguoi di moi
...
const updateBoardInvitation = async (req, res, next) => {
  try {
    ...
    const updatedInvitaion = await invitationService.updateBoardInvitation(userId, invitationId, status)
    res.status(StatusCodes.OK).json(updatedInvitaion)
  } catch (error) {next(error)}
}
